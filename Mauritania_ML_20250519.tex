...
{\bf ABSTRACT.} The advances in Machine Learning (ML) and computer technologies enabled to process satellite images using programming. Environmental applications that handle Remote Sensing (RS) data for spatial analysis use such approach, e.g., Python's library Scikit-Learn using algorithms on pattern identification, predictions, or image classification. This paper presents a ML method of satellite image processing using Geographic Resources Analysis Support System (GRASS) GIS. The aim is to classify multispectral Landsat images using ML for identification of changes in salt pan of west Mauritania, Africa over the period of 2014 to 2023. We defined 10 classes of land cover categories and perform analysis of geological, lithological and landscape setting, and then introduce the principles, algorithms, and processing of the ML methods of GRASS GIS. The following classification models were employed to implement image classification with training: Random Forest (RF), Decision Tree, Gradient Boosting and Support Vector Machine (SVM). The results were compared with clustering performed by k-means and maximum-likelihood discriminant analysis. The cartographic visualisation and validation was implement through accuracy analysis. Results for the best performing SVM model with 7 band input produced an overall accuracy of 76\% and RF model 73\% for a ten-class land cover classification, compared with 69\% for Decision Tree Classifier and 67\% for Gradient Boosting Classifier, respectively. The SVM model embedded in GRASS GIS generates robust land cover maps with good accuracy from multispectral satellite images.  The paper demonstrated an ML-based automated approach to satellite image processing which links Artificial Intelligence (AI) with cartographic tasks.
...
\newpar By utilizing Earth observation data, land cover change detection aims to visualise and monitor the dynamics of various patterns of the landscape as a linked human-environment system. For advanced monitoring, one of the important approaches for identification landscape dynamics in land cover types is the analysis of changes and the rate of exposure of landscapes to climate and environmental effects using multi-temporal geospatial remote sensing (RS) data. The ML is widely recognised as advanced computer application for processing RS data. besides, it has a powerful functionality as a branch of Artificial Intelligence (AI) \citep{YANG2024102087,RATHORE2016122,LEMENKOVA2025100180,CEDRIC2022100049}, including application of RS data processing. The advantages of the RS data for environmental and geological mapping has been discussed in many previous works (e.g., \citep{Parajuli,Niang2022,Ashpole}). Among the benefits of the RS data, such as satellite images \citep{GRANDJEAN2006220} mentions their cost-effectiveness for geologic exploration, since RS data enable precise coverage of vast regions, such as Africa regions, with essential data for recourse exploration. The need for satellite images for natural resources management gives rise to the question of their effective processing,
...
\vfill \vspace{1mm}
	...
	\begin{subfigure}[b]{.5\textwidth}
		...
		\caption{2023}
	\end{subfigure}%
\vspace*{20pt}\caption{Maps of rejection threshold probability for accuracy analysis of image classification by: chi square test (\textbf{a}) 2014, (\textbf{b}) 2017, (\textbf{c}) 2018, (\textbf{d}) 2020, (\textbf{e}) 2022, (\textbf{f}) 2023. Software: GRASS GIS. Source of maps: author.}\label{fig07}
\end{figure}
